...
1. **初始化项目：** 使用 `electron-vite` 或 `electron-builder` 模板快速搭建项目框架
2. **数据库设计：** 编写schema.sql，创建数据表
3. **实现MVP：** 先实现核心功能（手动添加、列表展示）
4. **逐步迭代：** 添加检索、文档上传等功能

推荐使用 `electron-vite` 模板：
```bash
npm create @quick-start/electron
```

选择Vue + TypeScript模板，会自动配置好Vite、Vue 3、TypeScript、Electron等环境。
