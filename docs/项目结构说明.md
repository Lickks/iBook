...
│   │   │   ├── bookshelfHandler.ts     #  书架操作的 IPC 处理
│   │   │   └── backupHandler.ts        #  备份与恢复操作的 IPC 处理
│   │   │
│   │   └── services/                   #  主进程业务服务
...
│           │   └── bookshelf.ts        #  书架状态（bookshelves, currentBookshelf）
│           │
│           ├── views/                  #  页面组件（路由级）
...
│           │   ├── Settings.vue        #  设置页（含备份与恢复）
│           │   ├── TagManagement.vue   #  标签管理页面（阶段十一新增）
│           │   ├── BatchImport.vue     #  批量导入页面（阶段十四新增）
...
- **职责**:
  ...
  - 暴露 book、document、search、stats、tag、bookshelf、backup 相关 API
  - 提供 TypeScript 类型定义（index.d.ts）
  - 隔离渲染进程和 Node.js 环境

#### IPC 处理器

##### `src/main/ipc/bookHandler.ts` 已完成
- **作用**: 书籍相关 IPC 处理
- **职责**:
  - book:create - 创建书籍
  - book:update - 更新书籍
  - book:delete - 删除书籍
  - book:getById - 根据ID获取书籍
  - book:getAll - 获取所有书籍
  - book:search - 搜索书籍
  - book:deleteBatch - 批量删除书籍

##### `src/main/ipc/documentHandler.ts`  已完成
- **作用**: 文档相关 IPC 处理
- **职责**:
  - document:selectFile - 选择文件对话框
  - document:upload - 上传文档
  - document:delete - 删除文档
  - document:open - 打开文档
  - document:countWords - 重新统计字数
  - document:getByBookId - 获取书籍的文档列表
  - document:update - 更新文档

##### `src/main/ipc/searchHandler.ts`  已完成
- **作用**: 搜索相关 IPC 处理
- **职责**:
  - search:youshu - youshu.me 搜索
  - search:batchSearch - 批量搜索

##### `src/main/ipc/statsHandler.ts`  已完成
- **作用**: 统计相关 IPC 处理
- **职责**:
  - stats:getOverview - 获取总体统计
  - stats:getCategoryStats - 获取类型统计
  - stats:getPlatformStats - 获取平台统计
  - stats:getStatusStats - 获取状态统计
  - stats:getMonthlyStats - 获取月度统计

##### `src/main/ipc/tagHandler.ts`  已完成（阶段十一）
- **作用**: 标签相关 IPC 处理
- **职责**:
  - tag:create - 创建标签
  - tag:update - 更新标签
...
##### `src/main/ipc/bookshelfHandler.ts`  已完成
- **作用**: 书架相关 IPC 处理
- **职责**:
...
##### `src/main/ipc/backupHandler.ts`  已完成
- **作用**: 备份与恢复相关 IPC 处理
- **职责**:
  - backup:create - 创建备份
  - backup:restore - 恢复备份
...
#### `src/main/services/cache.ts`  已完成（阶段十四）
- **作用**: 查询缓存服务
- **职责**:
  - LRU策略缓存，最大缓存100条记录
  - 缓存有效期5分钟
...
#### `src/main/services/backup.ts`  已完成
- **作用**: 数据备份服务
- **职责**:
  - 创建数据库和文件的完整备份
  - 备份进度回调支持
...
#### `src/main/services/restore.ts`  已完成
- **作用**: 数据恢复服务
- **职责**:
  - 从备份文件恢复数据库和文件
  - 恢复进度回调支持
...
- **作用**: 路由配置
- **职责**:
  - 定义路由规则（Home、BookDetail、AddBook、Statistics、Settings、TagManagement、BatchImport、BookshelfManagement）
  - 路由懒加载
  - 路由守卫（如有需要）
...
#### `src/api/bookshelf.ts`  已完成
- **作用**: 书架 API 封装
- **职责**:
  - 调用 `window.electronAPI.bookshelf.*`
  - 书架 CRUD 操作
  - 添加/移除书籍到书架
  - 获取书架统计信息
  - 获取书籍所属的书架列表

#### `src/api/backup.ts`  已完成
- **作用**: 备份与恢复 API 封装
- **职责**:
  - 调用 `window.electronAPI.backup.*`
  - 创建备份
  - 恢复备份
  - 验证备份文件
  - 获取备份信息
...
#### `src/views/BatchImport.vue`  已完成（阶段十四）
- **作用**: 批量导入页面
- **功能**:
  - 书名列表导入（支持多行输入，自动过滤括号内容）
  - 文件上传导入（支持 TXT、EPUB、PDF、MOBI、AZW、AZW3、DOC、DOCX）
  - 自动网络检索并显示预览
  - 检索进度和状态实时显示
  - 支持全选/取消全选
  - 支持"只选择检索成功"选项
  - 重新搜索功能（针对检索失败的书籍）
  - 批量导入（自动下载封面、获取详细信息、上传文件）

#### `src/views/BookshelfManagement.vue`  已完成
- **作用**: 书架管理页面
- **功能**:
  - 书架列表展示（包含默认书架和自定义书架）
  - 创建新书架
...
#### `src/views/Settings.vue`  已完成（阶段十三完善）
- **作用**: 设置页面
- **功能**:
  - 主题设置（集成 ThemeSelector 组件）
  - 视图模式设置（网格/列表）
  - **数据备份**（ 已完成）
    - 创建备份（包含数据库、文档、封面、设置）
    - 备份进度显示
...
#### `src/renderer/src/components/BookshelfDialog.vue`  已完成
- **作用**: 书架创建/编辑对话框
- **功能**:
  - 创建新书架
  - 编辑现有书架
...
#### `src/renderer/src/components/BookshelfSelector.vue`  已完成
- **作用**: 书架选择器组件
- **功能**:
  - 下拉选择当前书架
  - 显示所有书架（包括默认书架）
...
#### `src/renderer/src/components/AddToBookshelfDialog.vue`  已完成
- **作用**: 添加到书架对话框
- **功能**:
  - 选择目标书架
  - 批量添加书籍到书架
...
#### `src/renderer/src/components/VirtualList.vue`  已完成（阶段十四）
- **作用**: 主题选择器组件
- **职责**:
  - 提供明亮/暗黑/跟随系统三种主题选择
  - 显示当前选中的主题状态
